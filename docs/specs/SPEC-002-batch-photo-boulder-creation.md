...
- [x] **AC-14**: **Absolute Farbtreue & Persistenz beim Speichern und Veröffentlichen (Zero-Color-Mutation Guarantee)**:
  - **Sofortige Draft-Persistenz**: Jede Farbauswahl und Detailänderung im `BoulderBottomSheet` für Entwürfe (`draft`) wird beim Klick auf "Speichern" sofort in `STORAGE_KEY_WALL_BOULDERS` persistiert (`updateBoulderDetails`), sodass der Entwurf die gewählte Hallenfarbe beibehält, selbst wenn die Seite neu geladen oder im Batch veröffentlicht wird.
  - **Batch-Publish Erhalt**: `publishBatch` übernimmt lückenlos alle im Formular gewählten Farben, Namen, Notizen und Radar-Werte direkt in den `active`-Status, ohne Farben auf alte Initialwerte zurückzusetzen.
  - **Strikte Farbskalen-Auflösung im Bottom-Sheet**: `BoulderBottomSheet` verwendet dieselbe robuste mehrstufige Farbauflösung wie `WallPhotoCanvas` (IDs, Aliase wie `scale_6a_<color>`, normalisierte Farbnamen, Namenssuche, Font-Grades) und fällt niemals versehentlich auf die erste Skalenfarbe zurück, wenn eine Route bearbeitet wird.
  - **Kein Reset bei Re-Renders**: Der interne Formular-State des `BoulderBottomSheet` wird nur beim initialen Öffnen oder Wechsel des Boulders initialisiert und darf durch externe Hook-/Sync-Re-Renders während der geöffneten Bearbeitung nicht zurückgesetzt werden.
  - **Gym-Aware Last Color Persistence**: Sowohl bei Pin-Erstellung als auch beim Speichern im Bottom-Sheet wird die zuletzt gewählte Farbe gym-spezifisch (`setLastSelectedGradeScaleId(gymId, colorId)`) für die aktive Halle gespeichert.
  - **Entfernung destruktiver Migrationen**: Es dürfen keine automatischen Farbmutationen (wie `scale_6a_schwarz / lila -> beige`) existieren.
  - **Schutz vor Überschreiben im Cloud-Sync**: Der Aufwärts-Sync nach Supabase darf die lokale Farbskala einer Route nur dann auf eine Remote-UUID umschreiben, wenn eine echte, eindeutige Farb-Übereinstimmung vorliegt, und niemals bei einem Blind-Fallback auf die erste Hallenfarbe. *(Getestet in `tests/setterColorPersistence.test.tsx`)*
- [x] **AC-15**: **Garantierte Persistenz von Ausprägungen & Farben ohne ungewollte Mutationen (Zero-Mutation & Single-Action Save Guarantee)**:
  - **Kein Überschreiben existierender Routen durch `defaultGradeScaleId`**: `resolveScaleId` im `BoulderBottomSheet` darf für bestehende Routen (sowohl `active` als auch bereits konfigurierte `draft` Pins) unter keinen Umständen auf `defaultGradeScaleId` (die zuletzt für eine andere Route gewählte Farbe) zurückfallen. `defaultGradeScaleId` dient ausschließlich als Initialwert für frisch auf die Wand getippte Pins ohne vorherige Skalenzuweisung.
  - **Single-Action Immediate Save für bestehende Routen**: Beim Klick auf „Änderung übernehmen“ im Bottom-Sheet für aktive Routen wird die Änderung direkt und endgültig in allen lokalen Stores (`STORAGE_KEY_WALL_BOULDERS`, `gymStorage`) gespeichert und synchron nach Supabase übertragen (`syncBridge.syncBoulders`). Es ist kein zweiter Bestätigungsschritt über die untere Aktionsleiste („Änderungen speichern“) erforderlich, wodurch versehentliches Verwerfen ausgeschlossen ist.
  - **Lückenlose Persistenz der Ausprägungen (Radar-Attribute)**: Alle 6 Achsen des Radar-Charts (Maximalkraft, Kraft-Ausdauer, Technik, Balance, Koordination, Flexibilität) sowie der abwärtskompatible `kraft`-Alias werden beim Ändern und Speichern sofort atomar übernommen und sowohl lokal als auch in Supabase persistiert.
  - **Native RFC4122 v4 UUIDs für Drafts**: `createDraftBoulder` generiert von Beginn an vollwertige v4 UUIDs (`crypto.randomUUID()`). Dadurch entfällt das temporäre `draft_...` Präfix, wodurch lokale und Remote-IDs 1:1 übereinstimmen und `syncFromSupabase()` neu erstellte oder geänderte Routen niemals irrtümlich als verwaist löscht.
  - **Schutz lokaler Custom-Ausprägungen beim Cloud-Merge**: Beim Downward-Sync (`syncFromSupabase`) werden lokale, vom Schrauber festgelegte Ausprägungen und Namen nicht durch Standard-Default-Werte (z.B. all-3s oder "Unbenannter Boulder") aus unvollständig synchronisierten Remote-Zeilen überschrieben.
  - **Vollständige Sektor-Alias-Unterstützung im Batch-Publish & Sync**: `publishBatch` und `syncBouldersToSupabase` unterstützen Sektor-Aliase (`SECTOR_ALIAS_MAP`), sodass Entwürfe und Routen-Updates unabhängig von der verwendeten ID-Variante (`sec_6a_cave` vs. `41be4e25-...`) immer publiziert und synchronisiert werden. *(Getestet in `tests/setterColorPersistence.test.tsx`)*
